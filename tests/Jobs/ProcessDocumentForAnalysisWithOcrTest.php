...
describe(ProcessDocumentForAnalysis::class, function (): void {
    ...
    it('updates document analysis status to completed when successful', function (): void {
        ...
        // Act
        ProcessDocumentForAnalysisWithOcr::dispatch($this->documentAnalysis);

        // Assert job was dispatched
        Queue::assertPushed(ProcessDocumentForAnalysisWithOcr::class, fn (ProcessDocumentForAnalysisWithOcr $job): bool => $job->documentAnalysis->id === $this->documentAnalysis->id);

        // Test actual job execution
        $job = new ProcessDocumentForAnalysisWithOcr($this->documentAnalysis);
        $job->handle(new TesseractAnalyzer);
